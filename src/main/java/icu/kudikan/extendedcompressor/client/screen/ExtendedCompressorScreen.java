package icu.kudikan.extendedcompressor.client.screen;
...
public class ExtendedCompressorScreen extends BaseContainerScreen<ExtendedCompressorContainer> {
    public static final ResourceLocation BACKGROUND = ResourceLocation.fromNamespaceAndPath("extendedcompressor", "textures/gui/compressor.png");
    private ExtendedCompressorTileEntity tile;

    public ExtendedCompressorScreen(ExtendedCompressorContainer container, Inventory inventory, Component title) {
    ...
    public void init() {
        ...
        if (this.tile != null) {
            this.addRenderableWidget(new EnergyBarWidget(x + 7, y + 17, this.tile.getEnergy()::getEnergyStored, this.tile.getEnergy()::getMaxEnergyStored));
        }
    }
...
//        }
    ...
    private ExtendedCompressorTileEntity getTileEntity() {
    ...
    public int getMaterialBarScaled(int pixels) {
        int i = Mth.clamp(Ints.saturatedCast(this.getMaterialCount()), 0, this.getMaterialsRequired());
